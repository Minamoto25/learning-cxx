...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        ...
        for (unsigned int idx = 0; idx < total; ++idx) {
            unsigned int i0 = idx / (shape[1] * shape[2] * shape[3]);
            unsigned int i1 = (idx / (shape[2] * shape[3])) % shape[1];
            unsigned int i2 = (idx / shape[3]) % shape[2];
            unsigned int i3 = idx % shape[3];
            ...
            unsigned int other_idx = o0 * others.shape[1] * others.shape[2] * others.shape[3]
